const METHODS: { label: string; className: string }[] = [
  ...
  { label: "Carte bancaire", className: "bg-foreground/5 text-foreground/70 border-border/15" },
  { label: "À la livraison", className: "bg-vert-soft text-vert-deep border-secondary/30" },
];

/// Bandeau des moyens de paiement (couleurs de marque conservées).
export function PaymentStrip() {
  return (
    <section className="border-y-2 border-border bg-card py-5">
      <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-center gap-x-10 gap-y-4 px-4 sm:px-6">
        <p className="w-full text-center font-display text-sm font-bold uppercase tracking-widest text-foreground/50 sm:w-auto">
