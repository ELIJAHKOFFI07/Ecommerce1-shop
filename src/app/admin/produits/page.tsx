...
export default async function AdminProductsPage({ searchParams }: { searchParams: Promise<{ q?: string; page?: string }> }) {
  ...
  const [items, total] = await Promise.all([
    db.product.findMany({ where, select: { id: true, sku: true, title: true, price: true, images: true, active: true, stockVirtuel: true, stockDisponible: true, stockBureau: true, lowStockAlert: true }, orderBy: { createdAt: "desc" }, skip: (page - 1) * LIMIT, take: LIMIT }),
  ...
  return (
    <>
      ...
      {items.length === 0 ? (
        ...
        <Table head={["Produit", "Référence", "Prix", "Virtuel", "Disponible", "Bureau", "État"]}>
          {items.map((p) => (
            <tr key={p.id} className="hover:bg-muted">
              ...
              <td className={`${td} tabular`}>{p.stockVirtuel}</td>
              <td className={`${td} tabular ${p.stockDisponible <= p.lowStockAlert ? "font-semibold text-warning" : ""}`}>{p.stockDisponible}</td>
              <td className={`${td} tabular`}>{p.stockBureau}</td>
              <td className={`${td} text-sm`}>{p.active ? <span className="text-success">En vente</span> : <span className="text-muted-foreground">Retiré</span>}</td>
            </tr>
          ))}
        </Table>
      )}
