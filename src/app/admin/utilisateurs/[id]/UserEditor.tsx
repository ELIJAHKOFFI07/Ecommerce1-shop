...
type U = { id: string; name: string; phone: string; role: string; blocked: boolean };
type Perm = { slug: string; canView: boolean; canEdit: boolean };

export function UserEditor({ user, isSelf, isSuperAdmin, modules, permissions, canDelete }: { user: U; isSelf: boolean; isSuperAdmin: boolean; modules: { slug: string; name: string }[]; permissions: Perm[]; canDelete: boolean }) {
  const router = useRouter();
  const [f, setF] = useState({ name: user.name, phone: user.phone, role: user.role });
  ...
  async function save(e: React.FormEvent) {
    ...
    setMsg(null);
    try {
      await api(`/api/admin/users/${user.id}`, { method: "PATCH", json: { name: f.name, phone: f.phone || null, ...(isSuperAdmin && !isSelf ? { role: f.role } : {}) } });
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="space-y-3 p-5">
        ...
        {!isSelf && (
          <>
            <ActionButton path={`/api/admin/users/${user.id}`} method="DELETE" variant="ghost" confirm={`Supprimer définitivement le compte de ${user.name} ?`} redirect="/admin/utilisateurs" disabled={!canDelete} className="w-full">
              Supprimer le compte
            </ActionButton>
            {!canDelete && <p className="text-xs text-muted-foreground">Ce compte a des commandes : il se bloque, il ne se supprime pas.</p>}
          </>
        )}
      </Card>
    </div>
