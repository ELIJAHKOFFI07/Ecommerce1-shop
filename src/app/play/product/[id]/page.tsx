...
const BTN_PRIMARY =
  "card-hard press inline-flex w-full items-center justify-center gap-2 rounded-full bg-primary px-5 py-3 font-display text-sm font-bold text-primary-foreground transition-all hover:translate-x-1 hover:translate-y-1 hover:shadow-none disabled:cursor-not-allowed disabled:opacity-50";

/// Boutons secondaires : même grammaire, aplat neutre (paper / ink / vert).
const BTN_BASE =
  "card-hard-sm press inline-flex items-center justify-center gap-2 rounded-full px-4 py-2 font-display text-sm font-bold transition-all hover:translate-x-0.5 hover:translate-y-0.5 hover:shadow-none";
...
function Pill({
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-full border-2 border-border px-3 py-1 text-xs font-bold ${className}`}
    >
      {children}
    </span>
  );
...
export default function ProductPage({
  ...
  if (!product)
    return (
      <div className="card-hard mx-auto mt-10 max-w-md rounded-3xl bg-surface p-8 text-center">
    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-8 lg:grid-cols-[1fr_1.05fr] lg:items-start">
        ...
        <div className="flex gap-3">
            ...
            <div className="flex shrink-0 flex-col gap-2 overflow-y-auto">
              {images.map((img, i) => (
                <button
                  ...
                  className={`press h-16 w-16 shrink-0 overflow-hidden rounded-xl border-2 transition-all ${
                    i === imageIndex
                      ? "border-accent"
          ...
          <div className="card-hard min-w-0 flex-1 rounded-3xl bg-surface p-2">
            <div className="relative aspect-square overflow-hidden rounded-2xl bg-surface-2">
              ...
              {images.length > 1 && (
                <span className="absolute bottom-3 right-3 rounded-full border-2 border-border bg-foreground px-3 py-1 text-xs font-bold text-background">
        ...
        <div className="min-w-0 space-y-5">
          <div className="flex items-start justify-between gap-3">
            ...
            {product.shops && (
              <Link
                ...
                className="card-hard-sm press mt-1 flex shrink-0 items-center gap-2 rounded-2xl bg-surface px-3 py-2 transition-all hover:translate-x-0.5 hover:translate-y-0.5 hover:shadow-none"
              >
                {product.shops.logo_url ? (
                  ...
                  <img
                    ...
                    alt=""
                    className="h-8 w-8 shrink-0 rounded-xl border border-border object-cover"
                  />
                ) : (
                  <span className="grid h-8 w-8 shrink-0 place-items-center rounded-xl border border-border bg-primary text-primary-foreground">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {reviews.length > 0 ? (
              <a href="#avis" className="inline-flex items-center gap-1.5 rounded-full border-2 border-border bg-sun px-3 py-1 font-display text-xs font-bold text-foreground">
                <Star className="h-3 w-3 fill-current" /> {avg.toFixed(1)} · {reviews.length} avis
              </a>
            ) : (
              <span className="inline-flex items-center rounded-full border-2 border-border bg-surface px-3 py-1 text-xs font-medium text-muted">
                Aucun avis
              </span>
            )}
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex-1 min-w-0">
              ...
              {hasDiscount && (
                <span className="mt-1 inline-block rounded-full bg-secondary/20 px-3 py-1 text-sm font-bold text-secondary">
            ...
            {!isMine && (
              <div className="flex gap-2">
                <button
                  ...
                  className="card-hard press inline-flex items-center gap-2 rounded-full bg-primary px-5 py-3 font-display text-sm font-bold text-primary-foreground transition-all hover:translate-x-1 hover:translate-y-1 hover:shadow-none disabled:cursor-not-allowed disabled:opacity-50"
                >
                  {added ? (
                    <>
                ...
                <button
                  ...
                  className="card-hard-sm press grid h-12 w-12 shrink-0 place-items-center rounded-full bg-surface transition-all hover:translate-x-0.5 hover:translate-y-0.5 hover:shadow-none"
                >
              ...
              <div className="flex flex-wrap gap-2">
                {product.product_variants.map((v) => (
                  <button
                    ...
                    className={`card-hard-sm press rounded-full px-3.5 py-1.5 text-sm font-display font-bold transition-all disabled:opacity-40 ${
                      variantId === v.id
                        ? "translate-x-0.5 translate-y-0.5 bg-foreground text-background shadow-none"
                        : "bg-surface text-muted hover:translate-x-0.5 hover:translate-y-0.5 hover:shadow-none hover:text-foreground"
                    }`}
                  >
                    {v.name}
      ...
      {reviews.length > 0 && (
        <section id="avis" className="scroll-mt-24">
          <div className="flex flex-wrap items-end justify-between gap-3">
            ...
            <div className="flex items-center gap-2 rounded-full border-2 border-border bg-surface px-3 py-1.5">
          ...
          <div className="stagger mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {reviews.slice(0, 5).map((r) => (
              <article
                key={r.id}
                className="card-hard-sm flex flex-col gap-2 rounded-3xl bg-surface p-3.5"
              >
