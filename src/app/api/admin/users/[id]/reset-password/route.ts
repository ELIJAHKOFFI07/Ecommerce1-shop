...
/// Nouveau mot de passe temporaire envoyé par e-mail — jamais affiché à l'admin.
export const POST = withApi<{ params: Promise<{ id: string }> }>(async (req, { params }) => {
  const admin = await requirePermission("users", "edit");
  const id = uuid.parse((await params).id);
  const user = await db.user.findUnique({ where: { id }, select: { email: true, name: true, role: true } });
  ...
  await sendWelcome(user.email, user.name, temp);
